...
<script type="text/javascript">
    ...
    var pageDataSql = 'GoodsInfoMapper.getListByParamsByPage';
...
//    添加
    function add() {
        layerOpenFn('${showPagePath}','添加物资','base/goods/goods_infoPage',bodyW-100, bodyH);
    }

    //    修改
    function edit() {
        if($('#tb_departments').bootstrapTable('getSelections').length == 1){
            layerOpenFn('${showPagePath}','编辑物资', 'base/goods/goods_infoEditPage', bodyW-100, bodyH);
        }else {
    ...
    $('#btn_export').click(function () {
        var mmm = ['库房名称','','物资名称','物资类型状态','物资描述','物资产地','','','','物资类型名称'];
        var titleParams = mmm.join(",");
        window.open('${exportPath}'+$('#formSearch').serialize()+'&sql=GoodsInfoMapper.selectInfo&fileName=物资管理&titleParams='+titleParams);
    });
</script>
