...
<html>
    ...
    <title>线路信息</title>
   	...
   	<script type="text/javascript">
		...
		$(function(){
			...
			$('#bsId').combobox({//所属馈线
			url:des.webContextRoot +'line/queryLineMessage.action?queryPara.subsId='+'${resultMap.subId}'+'&queryPara.consId='+'${resultMap.consId}',
			valueField: 'lineId',
			textField: 'lineName'
			});
			$('#bsIdBak').combobox({//所属母线
			url:des.webContextRoot +'line/queryBSMessageBak.action?queryPara.subsId='+'${resultMap.subId}'+'&queryPara.consId='+'${resultMap.consId}',
			valueField: 'bsId',
			textField: 'bsName'
			});
			...
			$('#bsId').combobox({//所属馈线
				value:'${resultMap.F_LINE_ID}' 
			});
			$('#bsIdBak').combobox({//所属馈线
				value:'${resultMap.BI}' 
			});
			$('#validFlag').combobox({//是否有效
				value:'${resultMap.VF}'
			});
			/* $('#lineType').combobox({//是否有效
				value:'${resultMap.LT}'
			}); */
	       ...
	       /* $('#remark').textbox({
			...
			$('#bsId').combobox({
				onChange : function(){
					if(changeFlag){
						var lineId = $("#bsId").combobox('getValue');
						if(lineId != ""){
							$.ajax({
							...
							$('#bsIdBak').combobox({
								onChange : function(){
								var bsId = $("#bsIdBak").combobox('getValue');
								if(bsId != ""){
									$.ajax({
	...
	onClick : function(){
		...
		function doSubmit(result){
			if(result) {
				...
				"&bsIdBak="+$('#bsIdBak2').val();
				$.ajax({
					url : des.webContextRoot +'line/updatePowerMessage.action',
					type: "post",
					dataType:"text",
					data : para,
					timeout:60000, 
		   ...
		   // 画面合法性check
		  	function validateCheck() {
			     ...
			     if($('#lineName').textbox('getValue').length >64) {
			          $.messager.confirm("提示","用能设备名称不能超过64位，请重新输入");
			           return false;
			     }
			     /*if($('#bsIdBak').combobox('getValue') == "") {
			     ...
			     */
			     ...
			     if($('#showIndex').textbox('getValue') == "") {
			          $.messager.confirm("提示","用能设备显示序号不能为空，请输入");
			           return false;
			     }
			     
			     if($('#showIndex').textbox('getValue') < 1) {
			          $.messager.confirm("提示","用能设备显示序号不能小于1，请输入");
			           return false;
			     }
			     if(!onlyNumInput($('#showIndex').textbox('getValue')) ) {
			          $.messager.confirm("提示","用能设备显示序号只能为数字，请重新输入");
			           return false;
			     }
		    	return true;
			}
	...
	function modifyTmnlInfo(saveFlag){
		   ...
		   $("#bsIdBak").combobox({disabled:saveFlag});
	...
	<body srolling='no'>
		<div id='queryDiv' class="container-shadow container-marginTop" style="width:100%;">
		    ...
		    <input type='hidden' id='bsIdBak2' name='bsIdBak2' value="${resultMap.BI}">
            <div class="easyui-panel" title="用能设备信息" style="width:100%;padding:30px 10px;">
                <table style="width:100%;" class="easyui-panel" cellspacing="8px" cellpadding="0" border="0">
                    <tbody>
                             <tr>
                             	<td class="td-label"  align="right" nowrap="nowrap">设备类型：</td>
		                        <td>
		                        ...
		                        <td class="td-fillwidth"></td>
                             	...
                             	<td>
                                    <select class="easyui-combobox" id="bsIdBak" data-options="width:205,prompt:'请选择',panelHeight:'100',editable:false" disabled = 'disabled'></select>
                                   <!--   <font style="color: red;">*</font>-->
                                </td>
                                <td class="td-label" align="right">所属馈线：</td>
                             	<td>
                             ...
                             <tr>
								...
								<td width="18%">
									<input id="voltCode"   size="27" disabled = "disabled" editable='false' value="${resultMap.VOLTCODE}"> 
								</td>
                             ...
                             <tr>
                                ...
                                <td >
                                    <input  size="27" id="ptRatio"  disabled = "disabled"   value = "${resultMap.PTRATIO }"
                             ...
                             <!-- <tr>
								...
								<td >
									<input id="lineType" style="width:205px;" editable='false' disabled = 'disabled'> 
                                    <font style="color: red;">*</font>
								</td>
                             </tr> -->
                            <tr>
