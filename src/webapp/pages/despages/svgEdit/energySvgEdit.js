 ...
 * 新建svg代码
 */
var newSvgCode = '<svg width="1400" height="800" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="svg_Main" version="1.1" viewBox="0 0 1400 800">\n'
 ...
 */   
	 ...
	 * 删除按钮点击事件
	 */
	$("#DeleteEnergyMpAll").click(function() {
		if(!$('#DeleteEnergyMpAll').linkbutton("options")["disabled"])
		{			
			DeleteMpAll();
		}
	});
	$('#DeleteEnergyMpAll').linkbutton({disabled:true});
/**
 * 电压等级颜色设置按钮点击事件绑定
 */
	...
	$('#dialog2').dialog({
        ...
        onOpen: function () {
            if (window.frames[0].location.href == 'about:blank'){
                window.frames[0].location.href = basePath+"pages/despages/svgEdit/energySvgEditManage.jsp?isFirstLoad=0";
            	//window.location.href= basePath+"pages/despages/svgEdit/svgEditManage.jsp";
            }
//            setTimeout(function(){
//
//            	$(window.frames[0]).removeClass("hidden")
//            },300)
        },
        onClose:function(){
...
function IniSvgTools(svg)
{
	...
	$("#canvasBackground").children().attr("fill", "white");//设置背景颜色
...
function SvgIni(userTranId,unionSvgId,unionSvgName,unionSvg)
{
	if (unionSvgId != undefined){
		$('#svgSelectCtl').combobox({
			url : basePath+ 'energySvgManage/QuerySubSvgList.action?userTranId='+ userTranId,
			method : 'get',
			valueField : 'svgId',
			textField : 'svgName',
			multiple : false,// 是否支持多选
...
function DevIni(userTranId)
{
	$('#devSelectCtl').combotree({
		...
		url:basePath +'destree/queryYhbTree.action?treeState=open'
		 +'&isAllTreeNode=true&treeNodeType=1&checkNodeList='+devIdArr+'&id='+userTranId,
	    method:'get',
	    multiple:false,//是否支持多选
	    editable:'true',
...
function MPIni(devId,selectId,noClear)
{
    ...
    $('#mpGrid').datagrid({
		url : basePath
		+ 'energySvgManage/getMpInfo.action?tranId='
		+ devId,
...
function GetSvgSet(svgId) {
	if (svgId != null && parseInt(svgId) > 0) {
		$.ajax({
			type : "post",
			url : basePath + 'energySvgManage/querySVGset.action',
			dataType : "json",
			data : {
				'svgId' : svgId
			},
...
function GetSvgColorSetting(selectSvgId) {
	...
	if (selectSvgId != null && selectSvgId != "" && parseInt(selectSvgId) >= 1) {
		$.ajax({
			type : "post",
			url : basePath + 'energySvgManage/QuerySvgVolSetList.action',
			dataType : "json",
			data : {
				'svgId' : selectSvgId,
			},
...
function IniColorGrid(userTranId) {
	$("#volGrid").datagrid({
		url : basePath + 'energySvgManage/QueryVolListBySub.action?userTranId=' + userTranId
	});
}
...
function getSvg(id,unionSvg) {
	...
	if (typeof(unionSvg) != "undefined" && unionSvg != null){
		...
		$.ajax({
			type : "post",
			url : basePath + 'energySvgManage/GetSvgCode.action',
			dataType : "json",
			data : {
				'svg' : svgPath
			},
...
//		}
		$.ajax({
			type : "post",
			url : basePath + 'energySvgManage/queryEnergySvg.action',
			dataType : "json",
			data : {
				'svgId' : id
			},
...
//	$("#devSelectCtl").combobox({disabled:false});
	if (clickSetObj != "" && clickSetObj != null && clickSetObj.objId == $('#elem_id').val()){
		...
		if(clickSetObj.sfMpId != null || clickSetObj.sfMpId != "" ){
			$('#DeleteEnergyMpAll').linkbutton({disabled:false});
...
//		MPIni(clickSetObj.svgDevId);//测点表格初始化
		$("#mpChkBox").removeAttr("checked");//不选择绑定测点
//		MPChk_Click();//关联测点事件
	}
	//MPIni(svgSet[i].deviceId,svgSet[i].sfMpId,true);//测点表格初始化
...
//	$.messager.confirm('提示', "确定上传能效总览图?", function(r){
//		if(r){
//			$('#divWaiting').window('open'); 
//			divIswait ++;
//			$.ajax({
...
//							strDefs = "";
//						}
//						svgChart= "<svg width='"+wd+"' height='"+ht+"' id='svg_Main' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'><!-- Created with SVG-edit - http://svg-edit.googlecode.com/ -->" + strDefs+svgChart.substring(svgChart.indexOf("</title>")+8,svgChart.lastIndexOf("</g>"))+ "</svg>";					
...
//						var wd = $(svgChart).attr("width");
//						var ht = $(svgChart).attr("height");
//						if(svgChart.indexOf("<defs>") != -1 && svgChart.lastIndexOf("</g>") != 535){
//							var strDefs = "";
//							strDefs+= svgChart.substring(svgChart.indexOf("<defs>"),svgChart.lastIndexOf("</defs>")+7) 
//						}else{
//							strDefs = "";
...
//	});
	$.messager.confirm('提示', "确定上传一次图?", function(r){
		if(r){
			...
			{
				$("#tool_source").click();
				var svgChart1 = $("#svg_source_textarea").val();
				var wd = $(svgChart1).attr("width");
				var ht = $(svgChart1).attr("height");
				var strDefs = "";
				if(svgChart1.indexOf("<defs>") != -1 && svgChart1.indexOf("</defs>") !=382){
					strDefs+= svgChart1.substring(svgChart1.indexOf("<defs>"),svgChart1.lastIndexOf("</defs>")+7); 
				}
				var jiequSvgChart = svgChart1.substring(svgChart1.indexOf("<g"),svgChart1.lastIndexOf("</g>")+4);
				svgChart= "<svg width='"+wd+"' height='"+ht+"' id='svg_Main' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'><!-- Created with SVG-edit - http://svg-edit.googlecode.com/ -->" + strDefs+jiequSvgChart+ "</svg>";
				$("#tool_source_save").click();
			...
			$.getJSON(basePath + 'energySvgManage/uploadSvg.action', {
				'subSvgModel.svgId' : selectSvgId,
				'subSvgModel.userTranId' : userTranId,
				'subSvgModel.svgChart' : svgChart,
			}, function(json) {
...
//    		}
	    	...
	    	$.getJSON(basePath + 'energySvgManage/AddSvgMpSeting.action', {
...
//					}); 
	...
	$('#divWaiting').window('open'); 
	divIswait ++;
	$.getJSON(basePath + 'energySvgManage/AddSubSvg.action', {
...
function voltSettingColorBtnClick()
{
	...
	$.getJSON(basePath + 'energySvgManage/insertVoltColor.action', {
		'svgId' : selectSvgId,
...
function QuerySvgNameById(userTranId) {
	...
	$.ajax({
		type : "post",
		url : basePath + 'energySvgManage/QuerySubSvgList.action',
		dataType : "json",
		data : {
			'userTranId' : userTranId
		},
...
function deleteSvgBtnClick(){
	...
	$.messager.confirm('提示', "确定删除吗?", function(r){
		if(r){
			...
			$.ajax({
				type : "post",
				url : basePath + 'energySvgManage/deleteSvgBtnClick.action',
				dataType : "json",
				data : {
					'userTranId' : userTranId,
					'svgId' : selectSvgId
...
//					}); 
	...
	$.ajax({
		type : "post",
		url : basePath + 'energySvgManage/updateSvgBtnClick.action',
		dataType : "json",
		data : {
			'svgId' : selectSvgId,
			'svgName' : svgName	,
...
function reBack(){
//	window.frames[0].contentWindow.IniGridView("","");
	if (window.frames[0].location.href != 'about:blank' && window.frames[0].location.href != ""){
		window.frames[0].IniGridView("","");
	}
//	$(window.frames[0]).addClass("hidden")
//	window.frames[0].location.href = basePath+"pages/despages/svgEdit/energySvgEditManage.jsp?isFirstLoad=0";
//    setTimeout(function(){
...
//	var strokeColor = $("#selectorGrip_rotate").attr("stroke");
	...
	if(dSvgId != "" || dSvgId != null || dSvgId != undefined || dSvgId != "undefined"){
		$.messager.confirm('提示', "确定删除测点配置吗?", function(r){
			if(r){
				$.ajax({
					type : "post",
					url : basePath + 'energySvgManage/deleteEnergyMpAll.action',
					dataType : "json",
					data : {
						'objId' : dObjId,
						'svgId' : dSvgId
