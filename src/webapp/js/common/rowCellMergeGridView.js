...
Ext.ux.grid.RowCellMergeGridView = function(config){ 
    ...
    Ext.ux.grid.RowCellMergeGridView.superclass.constructor.call(this); 
}; 
 
Ext.extend(Ext.ux.grid.RowCellMergeGridView, Ext.util.Observable, { 
    /** 
     ...
     */ 
    rowSelector: 'div.x-grid3-group-row', 
    ...
    initTemplates : function(){ 
        ...
        if(!ts.allrow){ 
            ts.allrow = new Ext.Template( 
                    '<div><table class="x-grid3-row-table" border="0" cellspacing="0" cellpadding="0" style="{tstyle}">', 
                    '<tbody>{allrows}', 
                    '</tbody></table></div>' 
                    ); 
        }
        ...
        if(!ts.divrow){ 
            ts.divrow = new Ext.Template( 
                    '<div class="x-grid3-group-row {alt}" style="{tstyle}"><table class="x-grid3-row-table" border="1" cellspacing="0" cellpadding="0" style="{tstyle}">', 
                    '<tbody>{divrows}', 
                    '</tbody></table></div>' 
                    ); 
        }
        ...
        if(!ts.cell){ 
            ts.cell = new Ext.Template( 
                    '<td class="x-grid3-row-cell x-grid3-row-unmerge-cell x-grid3-col x-grid3-cell x-grid3-td-{id} {css}" style="{style}" colspan="{cols}" tabIndex="0" {cellAttr}>', 
        ...
        if(!ts.groupcell){ 
            ts.groupcell = new Ext.Template( 
                    '<td class="x-grid3-row-cell x-grid3-row-merge-cell x-grid3-col x-grid3-cell x-grid3-td-{id} {css}" style="{style}" colspan="{cols}" rowspan="{cellspans}" tabIndex="0" {cellAttr}>', 
                    '<div class="x-grid3-cell-inner x-grid3-col-{id}" unselectable="on" {attr}>{value}</div>', 
                    "</td>" 
                    ); 
        } 
 ...
 */ 
    ...
    // private
    getBorderWith : function(w, fit) {
    	var s = new String(w)
    	var tmp = s.substring(0, s.length - 2);
    	return (parseInt(tmp) + fit) + 'px';
    },
    ...
    updateAllColumnWidths : function(){ 
        ...
        for(var i = 0, len = ns.length; i < len; i++){ 
        	rowsIndexAr = [];
            ns[i].style.width = this.getBorderWith(tw, 2); 
            ns[i].firstChild.style.width = this.getBorderWith(tw, 2); 
            var row = ns[i].firstChild.rows[0]; 
            for(var j = 0; j < clen; j++){ 
                row.childNodes[j].style.width = this.getBorderWith(ws[j], -2);
                if (row.childNodes[j].rowSpan > 1) {
	                rowsIndexAr[rowsIndexAr.length] = j; 
                }
            }
            var indent;
            for (var j = 1; j < ns[i].firstChild.rows.length; j++) {
            	row = ns[i].firstChild.rows[j];
	            for(var k = 0; k < row.childNodes.length; k++){ 
	            	indent = this.getSubRowCol(rowsIndexAr, k);
	                row.childNodes[k].style.width = this.getBorderWith(ws[k + indent], -2); 
	            }
            }
        } 
    ...
    getSubRowCol : function (rowsIndexAr, col) {
    	var indent;
    	var rowsCol;
    	indent = 0;
    	for (var i = 0 ; i < rowsIndexAr.length; i++) {
    		rowsCol = rowsIndexAr[i];
    		if (col > rowsCol) {
    			indent = indent + 1;
    		}
    	}
    ...
    doRender : function(cs, rs, ds, startRow, colCount, stripe){ 
        var ts = this.templates, ct = ts.cell, gct = ts.groupcell, rt = ts.row, last = colCount-1, alrt = ts.allrow, drt = ts.divrow; 
        var tstyle = 'width:'+this.getBorderedTotalWidth()+';'; 
        // buffers 
        var buf = [], divrowsbuf = [], allrowsbuf = [], cb, c, p = {}, rp = {tstyle: tstyle}, divrp = {tstyle: tstyle},  alrp = {tstyle: tstyle}, r;
        var maxbaserow = 1;
        var baserow = 0;
        var rowcount = 0;
        for(var j = 0, len = rs.length; j < len; j++){ 
            ...
            var i = 0;
            var tmprow = parseInt(r.data["rowspan"]);
            if (tmprow != 0) {
            	baserow = tmprow;
            }
            //var baserow = 2;
            ...
            for(; i < colCount; i++){
                c = cs[i]; 
                cellrowspan = parseInt(r.data[c.name + "_rowspan"]);
                var tmpcols = r.data[c.name + "_colspan"];
                if (tmpcols != undefined) {
                	p.cols = parseInt(tmpcols);
                } else {
                	p.cols = 1;
                }
                ...
                if (rowcount != 0 && rowcount < baserow && cellrowspan == 1) {
                	//p.css += ' x-grid3-row-unmerge-cell';
                }
            	if (cellrowspan > 1) {
            		p.cellspans = cellrowspan;
	                cb[cb.length] = gct.apply(p);
            ...
            if (rowcount == baserow) {
	            divrp.alt = alt.join(" "); 
            	divrp.divrows = buf.join("");
            	divrowsbuf[divrowsbuf.length] = drt.apply(divrp);
            	rowcount = 0;
            }
        }
        //alrp.allrows = divrowsbuf.join("");
        //allrowsbuf[allrowsbuf.length] = alrt.apply(alrp);
        //return allrowsbuf.join(""); 
        return divrowsbuf.join("");
    }, 
    ...
    renderUI : function(){ 
        ...
        if(g.enableColumnResize !== false){ 
            this.splitone = new Ext.ux.grid.RowCellMergeGridView.SplitDragZone(g, this.mainHd.dom); 
        } 
    ...
    getBorderedColumnStyle : function(col, isHeader){ 
        var style = !isHeader ? (this.cm.config[col].css || '') : ''; 
        style += 'width:'+this.getBorderedColumnWidth(col)+';'; 
        if(this.cm.isHidden(col)){ 
            style += 'display:none;'; 
        } 
        var align = this.cm.config[col].align; 
    ...
    // private 
    getBorderedColumnWidth : function(col){ 
        var w = this.cm.getColumnWidth(col);
        var pw = 2;
        if(typeof w == 'number'){ 
            return (Ext.isBorderBox ? w : ((w-this.borderWidth > 0 ? w-this.borderWidth:0)) - pw) + 'px'; 
        } 
        return w; 
    }, 
    ...
    // private 
    getBorderedTotalWidth : function(){ 
        return (this.cm.getTotalWidth() + 2) + 'px'; 
    }, 
    ...
    getColumnData : function(){ 
        ...
        for(var i = 0; i < colCount; i++){ 
            ...
            cs[i] = { 
                ...
                id : cm.getColumnId(i), 
                style : this.getBorderedColumnStyle(i) 
            }; 
        } 
        return cs; 
    }, 
...
Ext.ux.grid.RowCellMergeGridView.SplitDragZone = function(grid, hd){ 
    this.grid = grid; 
    this.view = grid.getView(); 
    this.marker = this.view.resizeMarker; 
    this.proxy = this.view.resizeProxy; 
    Ext.ux.grid.RowCellMergeGridView.SplitDragZone.superclass.constructor.call(this, hd, 
...
Ext.extend(Ext.ux.grid.RowCellMergeGridView.SplitDragZone, Ext.dd.DDProxy, { 
        ...
        if(t){ 
            ...
            if(adjust !== false){ 
                ...
                if(this.cm.isResizable(this.cellIndex) && !this.cm.isFixed(this.cellIndex)){ 
                    Ext.ux.grid.RowCellMergeGridView.SplitDragZone.superclass.handleMouseDown.apply(this, arguments); 
                } 
