...
var startTime = DateUtil.dateAdd('m',-2,currentdate).getFullYear()+"-"+DateUtil.dateAdd('m',-2,currentdate).getMonth()+"-"+DateUtil.dateAdd('m',-2,currentdate).getDate();
//结束时间
...
//	        {field: 'checkbox', title: '复选框',checkbox: true}, 
	        ...
	        {field: 'dataDate', title: '日期', width:'8%',
	        	formatter: function(value,row,index){
				return value.substr(0,10);
	        	}},
	        {field: 'weather', title: '天气', width:'5%',
	        	formatter : function(value, row, index) {
				return HTMLEncode(value);
	        	}},
        	{field: 'jjbDate', title: '交接班日期', width:'8%',
	        	formatter: function(value,row,index){
				return value.substr(0,10);
	        	}},
	        {field: 'dealUser', title: '交班人', width: '6%',
				formatter : function(value, row, index) {
					return HTMLEncode(value);
				}},
	        {field: 'carryUser', title: '接班人', width: '6%',
				formatter : function(value, row, index) {
					return HTMLEncode(value);
				}},
...
//	IniDataGrid(timeType,handOverUserY,takeOverUserY,startTime,endTime);
	$('#workGrid').datagrid({
		title:'交接班管理',
		url:basePath+ 'changeShift/queryJtWork.action',		
...
//	$('#divPanel').window('open');
	$('#divPanel').window('center').panel('open');
...
function deleteWork(){
	...
	$.messager.confirm('提示', "确定删除吗?", function(r){
		if(r){
			$.ajax({
				type : "post",
				url : basePath + 'changeShift/deleteJtWork.action',
				dataType : "json",
				data : {
					'id' : ids,
				},
...
//	var dataDate = $("#dataDate").datebox('getValue');
	...
	//交班人
	var dealUser = $.trim($("#dealTkUser").textbox('getValue'));
	//接班人
	var carryUser = $.trim($("#carryTkUser").textbox('getValue'));
	//巡视记录
...
//	}
	...
	if(id == ""){
		...
		$.getJSON(basePath + 'changeShift/insertJtWork.action', {
		...
		$.getJSON(basePath + 'changeShift/updateJtWork.action', {
