...
<head>
		...
		var northPanel = new Ext.FormPanel({
						...
						Ext.Msg.confirm("确认","<br/>保存数据集合<b> "+title+"</b> ？<br/>",function(id){
							if(id==="yes"){
								northPanel.form.submit( {
									url : gdc.webContextRoot+'/dataSetPurview/addDataSetPurviewForArea.action?checkedNodes='+cns+"&excludeNodes="+ens,
          ...
          var  tree = new Tree.TreePanel({  
                ...
                var loader = new Tree.TreeLoader({  
                        dataUrl:gdc.webContextRoot+'/dataSetPurview/findResources.action'  
                    });
   ...
   **/
	...
	/***
	tree.on("textchange",function(node,text,oldText){alert(1);
	if(node.isExpanded()){
   		node.eachChild(function(child) {  
        	var id2 = child.id;
	  	 	var text2 = child.text;alert(text2);
	  	 	var i2 = text2.indexOf("[");
	  	 	var text2 = text2.substring(0,i2);
	  	 	var rw2 = "";
	  	 	if(v==2){
  ...
  var excludeNodes = function() {  alert("exclude...");
            ...
            var checkedNodes = function(){alert("checked...");
