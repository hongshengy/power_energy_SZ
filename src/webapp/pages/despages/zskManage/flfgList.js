...
//当前页数
var pageNo = 1;
var nl_type=1; //法律法规在附件表中所对应的类型
var zcfg_type=1;//文件类型
var editType = '';//修改或者增加类型
var $list = $("#xz_flfgFile"); //文件显示的ID位置
...
$(function(){	
	$('#flfg').datagrid({    
		title:'法律法规',
		url: webContextRoot +"knowledgeBase/selectFlfgInfo.action?policyLaw.zcfg_type=1",
		...
		singleSelect: true,
		columns:[[
		        {field:'pl_name',title:'法规名称',width:100,
		        	formatter: function(value,row,index){
					return HTMLEncode(value);
				}
	       	},		        
		       	{field:'type_name',title:'法规来源',width:60},
		...
		onLoadSuccess:function(data){
			var rows = $('#flfg').datagrid("getRows");
			if(rows.length>0){
				$('#flfg').datagrid("selectRow",0);
			}
		}
	});  
});
...
function bt_add(){
	...
	init_xz();
	$('#bt_add_commit').removeClass('webuploader-element-invisible');
//	$('#bt_update_commit').hide();
	$('#zcfgxz').dialog('setTitle','法律法规-新增');
	$('#zcfgxz').dialog('open');	
}
/**
 * 新增 提交
...
function bt_add_commit(){
	 ...
	 * 获取文件的ID
	 */
//	var flfgURL = $("#xz_flfgFile .flfgFile").attr("id");	//上传文件URL
//	var flfgFileName = $("#xz_flfgFile .flfgFile").attr("title");	//上传文件 名称 var zdbgFileName = $("#xg_zdbgFile .zdbgFile").text();
...
//	var nl_type=1;
	...
	var allData = $('#flfg').datagrid('getRows');
	var rowData = null ;
					...
					success : function(data) {
						if(data.FLAG == "1"){
							if(obj.ai_file_name.length>25){
								//文件名长度超过25，后面部分省略
								html='<a id="'+obj.ai_url+'" href="'+webContextRoot+obj.ai_url+'" target="_blank" class="flfgFile"  style="width: 120px; height: 30px;" title="'+obj.ai_file_name+'" download="'+obj.ai_file_name+'" >'+obj.ai_file_name.substr(0,24)+'...</a>';
							}else{
								//文件名长度不超过25，原样显示
								html='<a id="'+obj.ai_url+'" href="'+webContextRoot+obj.ai_url+'" target="_blank" class="flfgFile"  style="width: 120px; height: 30px;" title="'+obj.ai_file_name+'" download="'+obj.ai_file_name+'" >'+obj.ai_file_name+'</a>';
							}
						}else if(data.FLAG == "2"){
							if(obj.ai_file_name.length>25){
								//文件名长度超过25，后面部分省略
								html="<a id=\""+obj.ai_url+"\" href=\"#\" onclick=\"$.messager.alert('提示', '下载的文件不存在！', 'warning');\" class=\"flfgFile\"  style=\"width: 120px; height: 30px;\">"+obj.ai_file_name.substr(0,24)+"...</a>";
							}else{
								//文件名长度不超过25，原样显示
								html="<a id=\""+obj.ai_url+"\" href=\"#\" onclick=\"$.messager.alert('提示', '下载的文件不存在！', 'warning');\" class=\"flfgFile\"  style=\"width: 120px; height: 30px;\">"+obj.ai_file_name+"</a>";
							}
						}
					}
				});
				
					$("#ck_flfgFile").html(html);			
			});
		}
	});	
	$('#zcfgck').dialog('open');		
 ...
 */
function bt_delete(){
	var idlist = $('#flfg').datagrid('getSelected');
	if(idlist==null){
...
function bt_modify(){
	    ...
	    if(idlist==null){
			...
			$('#zcfgxg').dialog('setTitle','法律法规-修改');
			...
			$.ajax({	
				...
				success:function(result){
					$.each(result,function(index,obj){
						...
						html1 += noUpload;
						$("#xg_flfgFile").html(html1);
						editFlfgFileName = obj.ai_file_name;
						editflfgUrl = obj.ai_url;
						$("#xg_flfgUpload").addClass('webuploader-element-invisible');
						$(".del").click(function(){
							$(this).parent().html("");
							$("#xg_flfgUpload").removeClass('webuploader-element-invisible');
							isEdit = true;
						});		
					});
...
function bt_update_commit(){
	...
	if(uploadArray.length>0){
		...
		if($.trim($("#xg_flfgFile").text()).length>0 && isEdit == false){
...
//			if (error) {
				...
				$(".noUpload").click(function(){
					...
					$("#"+id.substr(0,index)).removeClass('webuploader-element-invisible');
				});
				return;
		},1,1);
	});
...
//	var flfgFileName = $("#xz_flfgFile").attr("title");
	if(editType == "A"){
		var children = $("#xz_flfgFile").children();
		...
		var pl_content = $("#pl_content1").val();//法规内容
		if(pl_name.length==0){
			$.messager.alert('提示','法规名称不能为空!','info');    
			return false;
		}
		if(pl_name.length>60){
			$.messager.alert('提示','法规名称应小于60个字!','info');    
			return false;
		}
		if(pl_content.length==0){
			$.messager.alert('提示','法规内容不能为空!','info');    
			return false;
		}
		if(pl_content.length > 500){
			$.messager.alert('提示','法规内容不能大于500个字!','info');    
			return false;
		}
		if(zcfg_xxly.length == 0){
			$.messager.alert('提示','法规来源不能为空!','info');    
			return false;
		}
		if(xxly_lynr == 0){
		...
		if(isEdit == true){
			if($("#xg_flfgFile").text().length>0){
				var children = $("#xg_flfgFile").children();
				if(children.length>0){
					var flfgFileId = "file"+children[0].id;
				}
				var flfgFileName = $("#"+flfgFileId).attr("title");
