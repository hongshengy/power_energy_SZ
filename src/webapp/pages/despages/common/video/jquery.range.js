...
//控件标签转换成时间
function spanToTime(values) {
    var ss1 = Math.floor(values);
    var ss2 = Math.round((values - ss1) * 60);

    var ss3 = PrefixInteger(ss2, 2);
    return ss1 + ":" + ss3;
}
//时间转换成标签
...
(function ($, window, document, undefined) {
    ...
    jRange.prototype = {
        ...
        //<div class="selected-default-r"></div>\<div class="selected-default-l"></div>\
        template: '<div class="slider-container">\
			<div class="back-bar">\
                <div class="selected-bar"></div>\
                <div class="pointer low"></div><div class="pointer-label"><input type="text" id="pointer_lable_left" value="5:00"/></div>\
                <div class="pointer high"></div><div class="pointer-label"><input type="text" id="pointer_lable_right" value="8:00"/></div>\
        ...
        showPointerValue: function (pointer, position, animate) {
            ...
            var texts;
            if (pointer[0] === this.highPointer[0]) {
                texts = '<input type="text" id="pointer_lable_right" value="' + text + '" onchange="updateLableP()">';//update by cxy
            } else {
                texts = '<input type="text" id="pointer_lable_left" value="' + text + '" onchange="updateLableP()">';//update by cxy
            }
            //var texts='<input type="text" id="pointer_lable_left" value="'+text+'" onchange="test('+value+')">';//update by cxy

            var width = label.html(texts).width(), //update by cxy
        ...
        positionToValue: function (pos) {
            ...
            var values = Math.round(value / this.options.step) * this.options.step; //update by cxy
            return spanToTime(values) //update by cxy
            //return values;
        },
    ...
    $.fn[pluginName] = function (option) {
        ...
        this.each(function () {
            ...
            if (!data) {
                $this.data('plugin_' + pluginName, (data = new jRange(this, options)));//入口
