        	...
        	if(countNum == 1){
        		 $('#cxTab').tabs({    
					    ...
					    onSelect:function(title,index){ 
					        ...
					        if(index == 0){
					        	var url = basePath+
				          		"/pages/areaEnergy/consDataCentre/subType/glysqxPic.jsp?LINEID="+lineId
				          		+"&dateTime="+datadate+"&station="+station;
					        	document.getElementById("pictableFrame").src = url;
					        }
					        if(index == 1){
					        	var queryUrl = basePath+"/userDataCenterAction/queryGlYSQXPicData.action?"
					        	+"queryPara.LINE_ID="+lineId+"&queryPara.DATE_TIME="+datadate
					        	+"&queryPara.QUERY_PIC=03&queryPara.QUERY_PARA=03&queryPara.startSize="+station;
	          ...
	          /**$('#cxTab').tabs({height:hheight});**/
	             ...
	             $('#cxTab').tabs({    
					    ...
					    onSelect:function(title,index){ 
					        ...
					        if(index == 0){
					        	var url = basePath+
				          		"/pages/areaEnergy/baseData/userDataCenter/glysqxPic.jsp?LINEID="+lineId
				          		+"&dateTime="+datadate+"&station="+station;
					        	document.getElementById("pictableFrame").src = url;
					        }
					        if(index == 1){
					        	var queryUrl = basePath+"/userDataCenterAction/glysqxData.action?"
					        	+"queryPara.LINE_ID="+lineId+"&queryPara.DATE_TIME="+datadate
					        	+"&queryPara.QUERY_TYPE=03&queryPara.QUERY_PARA=01&queryPara.startSize="+station;
					        	document.getElementById("sourceFrame").src = queryUrl;
