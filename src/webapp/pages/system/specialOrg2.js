Ext.namespace("com.frontier.gdc.system.specialOrg2");
	/**
	* 新建窗口
	* @type {Ext.Window}
	*/
	com.frontier.gdc.system.specialOrg2.newFormWin = null;
	/**
	* 编辑窗口
	* @type {Ext.Window}
	*/
	com.frontier.gdc.system.specialOrg2.editFormWin = null;
	
	com.frontier.gdc.system.specialOrg2.newFormSubWin = null;
...
*
*/
com.frontier.gdc.system.specialOrg2.initTree = function(){
	  var rootVisible = false;
	  if(gdc.currentRoleId==0){
	  		com.frontier.gdc.system.specialOrg2.rootVisible = true;	
	  }
	  var Tree = Ext.tree;
      tree = new Tree.TreePanel( {
			...
			animate : true,
			border : false,
			rootVisible : com.frontier.gdc.system.specialOrg2.rootVisible,	
	        listeners:{
				"click": function(node,e) {
							com.frontier.gdc.system.specialOrg2.node = node;
							if(!node.parentNode){
      ...
      var root = new Tree.AsyncTreeNode( {
            ...
            loader : new Tree.TreeLoader( {
                  dataUrl : gdc.webContextRoot+'/sysManageOrg/searchSpecialOrgTree.action' // 这就是要动态载入数据的请求地址，这里请求时会提交一个参数为node的值，值为root即new Tree.AsyncTreeNode()对象的id值
            	  ,listeners:{
...
* 初始化数据集合信息grid布局
*/
com.frontier.gdc.system.specialOrg2.initGrid = function(){
		/**
		 *新增Menu
		 * @type {Ext.menu.Menu}
		 */
		com.frontier.gdc.system.specialOrg2.addmenu = new Ext.menu.Menu({
						...
						handler:function() {
							...
							if(node && node.parentNode){
								if(node.parentNode == tree.getRootNode()){
									nodeId = 0;
									if(!com.frontier.gdc.system.specialOrg2.rootVisible){
...
//			}),
			url : gdc.webContextRoot+'/sysManageOrg/searchSpecialOrgList.action',
			reader : _jsonReader,
			listeners:{
		...
		grid = new Ext.grid.GridPanel( {
			...
			tbar : [ {
				...
				iconCls : 'add',
				bizCode:'add',
				menu: com.frontier.gdc.system.specialOrg2.addmenu,
				handler : function() {
...
var exportExl = function(){
	...
	if(nodeId){
		window.location.href=gdc.webContextRoot+'/sysManageOrg/exportExl.action?specialOrgId='+nodeId;
	}else{
...
* 厂站管理页面初始化
*/
com.frontier.gdc.system.specialOrg2.newMainPanel = function(){

	Ext.QuickTips.init();
	com.frontier.gdc.system.specialOrg2.initTree();
	com.frontier.gdc.system.specialOrg2.initGrid();
...
var newForm = new Ext.form.FormPanel( {
				...
				{id : 'New',
				 ...
				 handler : function() {
					if (newForm.form.isValid(newForm)) {
						if(com.frontier.gdc.system.specialOrg2.node.attributes.iconCls){
							if(com.frontier.gdc.system.specialOrg2.node.attributes.iconCls.indexOf('area')>-1){
								if(Ext.getCmp('addOrgType').getValue()==7){
									Appframe.viewinfdlg.warning.show("【"+com.frontier.gdc.system.specialOrg2.node.text+"】下不能添加组织类型为部门的组织");
									return;
								}
							}else{
								if(Ext.getCmp('addOrgType').getValue()!=7){
									Appframe.viewinfdlg.warning.show("【"+com.frontier.gdc.system.specialOrg2.node.text+"】下只能添加组织类型为部门的组织");
									return;
								}
							}
						}
						newForm.form.submit( {
							url : gdc.webContextRoot+'/sysManageOrg/addSpecialOrg.action',
							params:{"specialOrgLocal.porgName":Ext.getCmp('addPorgNoName').getRawValue()},
							success : function(form, action) {
								newForm.form.reset();
								if(com.frontier.gdc.system.specialOrg2.newFormWin)
								com.frontier.gdc.system.specialOrg2.newFormWin.hide();
								if(com.frontier.gdc.system.specialOrg2.newFormSubWin)
								com.frontier.gdc.system.specialOrg2.newFormSubWin.hide();
	...
	var newSpecialOrgDataSet = function() {
		//if (!com.frontier.gdc.system.specialOrg.newFormSubWin) {
			com.frontier.gdc.system.specialOrg2.newFormSubWin = new Ext.Window( {
				layout : 'fit',
				width : 400,
				height : 350,
				closeAction : 'hide',
		...
		//alert('nodeId '  + nodeId);
		v.setValue(nodeId);
		com.frontier.gdc.system.specialOrg2.newFormSubWin.show('New');
	};
	/**
	 * 新增同级业务组织
	 */
	var newParSpecialOrgDataSet = function() {
		//if (!com.frontier.gdc.system.specialOrg.newFormWin) {
			com.frontier.gdc.system.specialOrg2.newFormWin = new Ext.Window( {
				layout : 'fit',
				width : 400,
				height : 350,
				closeAction : 'hide',
		...
		//alert(nodeId);
		v.setValue(nodeId);
		com.frontier.gdc.system.specialOrg2.newFormWin.show('New');
	};
								...
								if(com.frontier.gdc.system.specialOrg2.node.attributes.iconCls){
									if(com.frontier.gdc.system.specialOrg2.node.attributes.iconCls.indexOf('area')>-1){
										if(Ext.getCmp('orgType').getValue()==7){
											Appframe.viewinfdlg.warning.show("【"+com.frontier.gdc.system.specialOrg2.node.text+"】下组织的组织类型不能修改为部门");
											return;
										}
									}else{
										if(Ext.getCmp('orgType').getValue()!=7){
											Appframe.viewinfdlg.warning.show("【"+com.frontier.gdc.system.specialOrg2.node.text+"】下组织的组织类型只能部门");
											return;
										}
									}
								}
								editForm.form.submit( {
									url : gdc.webContextRoot+'/sysManageOrg/updateSpecialOrg.action',
									params:{"specialOrgLocal.porgName":Ext.getCmp('editPorgNoName').getRawValue()},
									success : function(form, action) {
										com.frontier.gdc.system.specialOrg2.editFormWin.hide();
  ...
  */
	var editSpecialOrg = function() {	
		...
		if (!_record) {
			...
			if (!com.frontier.gdc.system.specialOrg2.editFormWin) {
				com.frontier.gdc.system.specialOrg2.editFormWin = new Ext.Window( {
					layout : 'fit',
					width : 450,
					height : 350,
					closeAction : 'hide',
			...
			com.frontier.gdc.system.specialOrg2.editFormWin.show('Edit');
			//部门
				...
				Ext.MessageBox.confirm('提示', '你确认要删除这条数据吗？', function(btn) {
					if (btn == "yes") {
						try{
							...
							Ext.Ajax.request({
								url:gdc.webContextRoot+'/sysManageOrg/deleteSpecialOrg.action',
		...
		var synchronizedSpecialOrg = function() {
				Ext.MessageBox.confirm('提示', '你确认要同步业务组织数据吗？', function(btn) {
					if (btn == "yes") {
						...
						Ext.Ajax.request({
							url:gdc.webContextRoot+'/sysManageOrg/synchronizedSpecialOrg.action',
							timeout : 300000,
							success : function(result,request) {
 ...
 * @type 
 */
com.frontier.gdc.system.specialOrg2.mdlCfg={
		mustRole:false,
		funcBtnControl:true
};	
