...
//	});
	...
	$('#typeNews').combobox({ 
		url:webContextRoot +'notice/queryNewCode.action?codeSortId=70045',
		valueField: 'codeValue',
		textField: 'codeName' ,
		editable:'false' ,
		onChange: function(newValue, oldValue){
 ...
 */
  function queryGjxxpzData(){
	  ...
	  var gridCommon = [[
             {field:'newsName',title:'公告标题',width:'30%',align:'left',formatter: function(v,row,index){ //超链接
...
//	 		{field:'newsName',title:'标题',width: 200,align:'left'},
	 		{field:'newsTypeName',title:'公告类型',width: 200,align:'left'},
	...
	$('#gjxxpz-datagrid').datagrid({// 表格
		...
		url:webContextRoot +'notice/queryNews.action',//新闻列表
		queryParams:{
			'ceInfoNews.newsId' : null,
			'ceInfoNews.newsName' : newTitle,//新闻名称
			'ceInfoNews.newsType' : newType//新闻类型
...
function addGjpzDate(updateObjects){
	if(editType == 'A'){
		...
		//新增操作
		 $.getJSON(webContextRoot + 'notice/saveCeInfoNews.action',
					{ 
