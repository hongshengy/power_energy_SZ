        ...
        <div class="panel-body">
            <form id="formSearch" class="form-horizontal">
                <div class="form-group" style="margin-top:15px">
                    <label class="control-label col-sm-1 text-nowrap" for="use_Date">物资领用时间</label>
                    <div class="col-sm-3">
...
<script type="text/javascript">
    ...
    var pageDataSql = 'GoodsUseInfoMapper.getListByParamsByPage';
    ...
    var columns = [
        {
            ...
            field: 'goods_use_Content',
            title: '物资领用备注'
        }, {
            field: 'goods_use_Remark',
            title: '物资领用说明'
        }
    ];
...
//    父子表gird显示参照fatherAndSonGird.jsp
    ...
    //    添加
    function add() {
        layerOpenFn('${showPagePath}','添加菜单','base/goods/goods_useinfoPage',bodyW-100, bodyH);
    }

    //    修改
    function edit() {
        if($('#tb_departments').bootstrapTable('getSelections').length == 1){
            layerOpenFn('${showPagePath}','编辑菜单', 'base/goods/goods_useinfoEditPage', bodyW-100, bodyH);
        }else {
    ...
    //    删除
    function deleteFun() {
        sysDeleteFun('GoodsUseInfoMapper.deleteInfo');
    }

    //    导出
    $('#btn_export').click(function () {
        var mmm = ['物资领用备注','库房名称','','','物资领用数量','物资领用人','物资名称','','','物资领用时间','物资领用说明','物资类型名称'];
        var titleParams = mmm.join(",");
        window.open('${exportPath}'+$('#formSearch').serialize()+'&sql=GoodsUseInfoMapper.selectInfo&fileName=物资领用管理&titleParams='+titleParams);
    });
</script>
