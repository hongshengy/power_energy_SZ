...
<script type="text/javascript">
    ...
    $.validator.setDefaults({
        submitHandler: function () {
            ...
            getDataFnNoErrorF(
                ...
                , function (data) {
                    debugger;
                    if (data[0][queryData.sql].length > 1 ) {
                        window.top.layerMsg('已存在相同数据，字典英文名不可重复，字典值不可重复');
                    } else {
                        if(data[0][queryData.sql][0]['id'] != selectRowData['id']){
                            window.top.layerMsg('已存在相同数据，字典英文名不可重复，字典值不可重复');
                        }else{
                            subData();
                        }
    ...
    function subData() {
        layerConfirm('确认提交？', ['确认', '取消'], [function () {
            getDataFnNoErrorF(
                updateDataPath
                , $('#formSearch').serialize() + '&sql=DictionaryInfoMapper.updateinfo&id=' + selectRowData['id']
                , function (data) {
