...
<html>
	...
	$(function() {
		$('#collArchiveTable').datagrid({
			...
			fitColumns: true, 
			url : des.webContextRoot+'line/queryPowerDevice.action',
			...
			toolbar : [
			{
				...
				handler : function() {
					...
					var url="<%=basePath %>/pages/areaEnergy/line/addNewPowerDevice.jsp?subId="+subId+"&dataTotal="+dataTotal+'&consId='+consId+"&roleCode="+'${param.roleCode}';
		    		OpenWin(url,"新增用能设备信息",1200,550);
				}
			},'-',{
					text:'修改用能设备信息',
					id : 'editLine',
					...
					handler:function(){
					    ...
					    if(rows.length==0){  
				            $.messager.alert('提示',"请选择你要用能设备的线路",'info'); 
				            return; 
				        }
				        if(rows.length > 1){
				        	$.messager.alert('提示',"请选择一条用能设备进行修改",'info'); 
				            return;
				        } 
				        var showIndex ;
				        var lineName;
				        ...
				        var tmnlId ;
			    		 ...
			    		 parent.com.frontier.areaEnergy.baseData.reloadTree.clickTreeNode(lineId+',POWERDEVICESELF,'+subId);
				    }
			},'-',{
					text:'删除用能设备信息',
					id : 'delLine',
					...
					handler:function(){
						var rows = $('#collArchiveTable').datagrid('getSelections'); 
					    if(rows.length==0){  
				            $.messager.alert('提示',"请选择你要修改的用能设备",'info'); 
				            return; 
				        }
				        if(rows.length > 1){
				        	$.messager.alert('提示',"请选择一条用能设备进行修改",'info'); 
		...
		$('#collArchiveTable').datagrid('hideColumn','LINE_TYPE');
		$('#collArchiveTable').datagrid('hideColumn','BS_ID');
