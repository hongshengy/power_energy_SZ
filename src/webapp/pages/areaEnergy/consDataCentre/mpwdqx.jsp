        ...
        Ext.onReady(function(){
             $('#cxTab').tabs({    
				    ...
				    onSelect:function(title,index){
				        ...
				        if(index == 0){
				        	var url = basePath+
			          		"/pages/areaEnergy/baseData/userDataCenter/mpwdqxPic.jsp?LINEID="+lineId
			          		+"&dateTime="+datadate+"&station="+station;
				        	document.getElementById("pictableFrame").src = url;
				        }
				        if(index == 1){
				        	var queryUrl = basePath+"/userDataCenterAction/mpwdqxData.action?"
				        	+"queryPara.LINE_ID="+lineId+"&queryPara.DATE_TIME="+datadate
				        	+"&queryPara.QUERY_TYPE=03&queryPara.QUERY_PARA=02&queryPara.startSize="+station;
