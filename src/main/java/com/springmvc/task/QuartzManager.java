 ...
 * @author 王正毅男
 * @date 2017年8月25日
 * @file com.springmvc.task.QuartzManager.java
 * @content quartz静态定时器管理类
 */
public class QuartzManager {
	...
	public static void addJob(Scheduler sched, String jobName, Class cls, String time,String params) {
        try {
            JobDetail jobDetail = new JobDetail(jobName, JOB_GROUP_NAME, cls);// 任务名，任务组，任务执行类
            jobDetail.getJobDataMap().put("params", params);
            //jobDetail.isVolatile()
            // 触发器
    ...
    @SuppressWarnings("rawtypes")
    public static void modifyJobTime(Scheduler sched, String jobName, String time,String params,String o) {
        try {
            CronTrigger trigger = (CronTrigger) sched.getTrigger(jobName, TRIGGER_GROUP_NAME);
            if (trigger == null) {
                return;
            ...
            if (!oldTime.equalsIgnoreCase(time)) {
                ...
                Class objJobClass = jobDetail.getJobClass();
                removeJob(sched, jobName);
                addJob(sched, jobName, objJobClass, time,params);
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
