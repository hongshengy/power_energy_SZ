...
.textbox .textbox-text{
	...
	<body  srolling='no'>
								...
								<table width="100%" cellspacing="8px" cellpadding="0"
									...
									<tbody>
										<tr>
											...
											<td>
												<input id="runStatus" name="queryPara.runStatus"
													size="27" value="${resultMap.RUN_STATUS}"validType="comboxValidate['runStatus','请选择运行状态']"
													disabled="disabled">
												<font style="color: red;">*</font>
											</td>
											<td class="td-label" align="center"nowrap="nowrap">
										...
										<tr>
											...
											<td>
												<input id="voltCode" name="queryPara.voltCode" validType="comboxValidate['voltCode','请选择电压等级']"
											...
											<td colspan="1">
												<input id="electSumFlag" name="queryPara.electSumFlag" validType="comboxValidate['electSumFlag','请选择用电量累加方式']"
										...
										<tr>
											...
											<td colspan="5">
												<input class="easyui-textbox" multiline="true" id="remark" data-option="validType:'length[0,128]',multiline:true"
													...
													disabled="disabled" style="width: 96%; height: 120px;">
		...
		<!-- <div style="border: 0; height:560px; overflow: auto; background: #fafafa;padding:0">
...
<script type="text/javascript">
    $(function() {
        ...
        $('#orgNo').combobox({
           url :'<%=basePath%>areaEnergyTmnl/loadOrgNoList.action?areaNo='+'${resultMap.AREA_NO}',  
           editable:false,//不可编辑状态
           valueField:'id',
           textField:'text'   
       });
       ...
       <%--  $('#roomTable').datagrid({
			height : $(window).height()-$('#pfgd-panel').height(),
			title:'房间列表',
			border : false,
			singleSelect : false,
			width:'100%',
			...
			fitColumns: true, 
			url : des.webContextRoot+'room/queryRoomList.action',
			...
			columns : [[
				...
				toolbar : [
				{
					...
					handler : function() {
						var consId = '${consId}';
						var url="<%=basePath %>pages/areaEnergy/capacity/room/addRoom.jsp?consId="+consId;
			    		OpenWin(url,"新增房间",460,160);
					}
				},'-',{
						text:'编辑房间',
						id : 'editRoom',
						...
						handler:function(){
						    ...
						    if(rows.length==0){  
					            $.messager.alert('提示',"请选择你要编辑的房间",'info'); 
					            return; 
					        }
					        if(rows.length > 1){
					        	$.messager.alert('提示',"请选择一条房间进行编辑",'info'); 
					            return;
					        }
					        var id ;
					        var name;
					         ...
					         var consId = '${consId}';
						     var params = "id="+id+"&name="+encodeURIComponent(name)+"&consId="+consId;
						     var url="<%=basePath %>pages/areaEnergy/capacity/room/editRoom.jsp?"+params;  
						     OpenWin(url,"编辑房间",460,160);
					    }
				},'-',{
						text:'删除房间',
						id : 'delRoom',
						...
						handler:function(){
						    var rows = $('#roomTable').datagrid('getSelections'); 
						    if(rows.length==0){  
					            $.messager.alert('提示',"请选择你要删除的房间",'info'); 
					            return; 
					        }
						    $.messager.confirm('提示','确定要删除吗?',function(result){  
						            if (result){  
						                ...
						                //删除客户
						                var url = "<%=basePath %>room/deleteRoom.action";
			...
			onLoadSuccess:function(){
					var isEdit = parent.$("#isEdit").val();
					if(isEdit == 'false'){
						$("#editRoom").linkbutton("disable");
						$("#addRoom").linkbutton("disable");
						$("#delRoom").linkbutton("disable");
					}
				}
			}); --%>
    });
