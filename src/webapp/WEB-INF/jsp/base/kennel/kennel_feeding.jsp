        ...
        <div class="panel-body">
            <form id="formSearch" class="form-horizontal">
                <div class="form-group" style="margin-top:15px">
                    <label class="control-label col-sm-1" for="feed_Type">饲料品种</label>
                    <div class="col-sm-3">
...
<script type="text/javascript">
    ...
    var pageDataSql = 'KennelFeedingMapper.getListByParamsByPage';
...
//            user_Name: $("#user_Name").val(),
    ...
    $('#tb_departments').bootstrapTable('hideColumn', 'ill_If');     //隐藏需替换的列
...
//    添加
    function add() {
        layerOpenFn('${showPagePath}','添加日常饲养记录','base/kennel/kennelfeedingPage',bodyW-100, bodyH);
    }

    //    修改
    function edit() {
        if($('#tb_departments').bootstrapTable('getSelections').length == 1){
            layerOpenFn('${showPagePath}','编辑日常饲养记录', 'base/kennel/kennelfeedingEditPage', bodyW-100, bodyH);
        }else {
    ...
    //    删除
    function deleteFun() {
        sysDeleteFun('KennelFeedingMapper.deleteInfo');
    }

    //    导出
    $('#btn_export').click(function () {
        var mmm = ['','饲养日期','有无病历','系统录入时间','','饲料品种','精神状态','毛色'];
        var titleParams = mmm.join(",");
        window.open('${exportPath}'+$('#formSearch').serialize()+'&sql=KennelFeedingMapper.selectInfo&fileName=日常饲养记录管理&titleParams='+titleParams);
    });
</script>
