...
function getDraggingDialog(e){
    var target=e.target;
    while(target && target.className.indexOf('dialog-Movetitle')==-1){
                ...
                &&myCharts._option.series[j].yAxisIndex==yAxisIndex)
                {
                  ...
                  else
                  {
                  	...
                  	if(l_isNaNMin.toString()!='NaN')
                  	{
                  		 //当前值小于最小值
              	        //把当前值赋值给最小值
	                     if((parseFloat(l_MinAndMax[0])>parseFloat(l_tempMinAndMax[0]) && !(l_tempMinAndMax[0]=='0' && l_tempMinAndMax[1]=='1'))
	                     ||(l_MinAndMax[0]=='0' && l_MinAndMax[1]=='1')||(l_MinAndMax[0]=='' && l_MinAndMax[1]==''))//假如上一次赋值为默认的0,1 作废
	                     {
                  	...
                  	if(l_isNaNMax.toString()!='NaN')
                  	{
	                     ...
	                     ||l_MinAndMax[1]=='1'||l_MinAndMax[1]=='')//假如上一次赋值为默认的0,1 作废
	                     {
