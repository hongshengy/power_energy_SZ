  			  ...
  			  //小储能
	       	  $('#cxTab').tabs({    
				    ...
				    onSelect:function(title,index){  
				        ...
				        if(index == 0){
				        	var url = basePath+
			          		"/pages/areaEnergy/consDataCentre/subType/ygglqxPic.jsp?LINEID="+lineId
			          		+"&dateTime="+datadate+"&station="+station;
				        	document.getElementById("pictableFrame").src = url;
				        }
				        if(index == 1){
				        	var queryUrl = basePath+"/userDataCenterAction/queryYGGLQXPicData.action?"
				        	+"queryPara.LINE_ID="+lineId+"&queryPara.DATE_TIME="+datadate
				        	+"&queryPara.QUERY_PIC=00&queryPara.QUERY_PARA=01&queryPara.startSize="+station;
	          ...
	          /**$('#cxTab').tabs({height:hheight});**/
	           ...
	           $('#cxTab').tabs({    
				    ...
				    onSelect:function(title,index){  
				        ...
				        if(index == 0){
				        	var url = basePath+
			          		"/pages/areaEnergy/baseData/userDataCenter/ygglqxPic.jsp?LINEID="+lineId
			          		+"&dateTime="+datadate+"&station="+station;
				        	document.getElementById("pictableFrame").src = url;
				        }
				        if(index == 1){
				        	var queryUrl = basePath+"/userDataCenterAction/ygglqxData.action?"
				        	+"queryPara.LINE_ID="+lineId+"&queryPara.DATE_TIME="+datadate
				        	+"&queryPara.QUERY_TYPE=01&queryPara.QUERY_PARA=01&queryPara.startSize="+station;
				        	document.getElementById("sourceFrame").src = queryUrl;
