 ...
 */
/**用户消息配置*/
Ext.namespace("com.frontier.gdc.system.noticeRoleManagerV2");

//******************************************************数据集合树*******************************************
//弹出数据集合树window
com.frontier.gdc.system.noticeRoleManagerV2.showNoticeTreeWindow = function(){
	//获取grid选中项
	...
	if (record) {
		...
		com.frontier.gdc.system.noticeRoleManagerV2.roleUserRoleId = roleId;
	...
	if(com.frontier.gdc.system.noticeRoleManagerV2.dataTreeWindow){
	    com.frontier.gdc.system.noticeRoleManagerV2.dataTreeWindow.close();
	}
               	...
               	listeners:{
               		"checkchange": function(node, checked) {
               			if(node.getDepth() != 2)
               			{
							...
							if(childs)
							{
								com.frontier.gdc.system.noticeRoleManagerV2.setNodeCheckStatus(node,childs);
							}
               			}
               			else
               			{
	...
	com.frontier.gdc.system.noticeRoleManagerV2.dataTreeWindow = new Ext.Window({
				title: '角色消息授权',
				layout: 'fit',
				modal: true,
				resizable: false,
	...
	com.frontier.gdc.system.noticeRoleManagerV2.dataTreeWindow.show();
	
}
//设置tree的checkbox的选中状态
com.frontier.gdc.system.noticeRoleManagerV2.setNodeCheckStatus=function(node,childs)
{
		for(var n=0;n<childs.length;n++)
		{
			...
			if(cchilds)
			{
				com.frontier.gdc.system.noticeRoleManagerV2.setNodeCheckStatus(childs[n],cchilds);
			}
		}
}
//用户消息配置保存
com.frontier.gdc.system.noticeRoleManagerV2.noticeRoleSubmit = function(){
    var checkedNodes = com.frontier.gdc.system.noticeRoleManagerV2.tree.getChecked();
    var startTime = "";
    var endTime = "";
    if(com.frontier.gdc.system.noticeRoleManagerV2.isMessage == 1){
	...
	Ext.Ajax.request({
		...
		params : {"roleId" : com.frontier.gdc.system.noticeRoleManagerV2.roleUserRoleId,"noticeRoleJson" : json},
		success : function(result,request) {
			var res = result.responseText;
			var tem = eval('('+res+')');
			if(tem.success==true){
				com.frontier.gdc.system.noticeRoleManagerV2.dataTreeWindow.hide();
...
//********************************************树loader设置*****************************************************
//加载loader
com.frontier.gdc.system.noticeRoleManagerV2.loader = new Ext.tree.TreeLoader({
	url : gdc.webContextRoot+"noticeRoleTree/findNoticeTree.action",
	listeners:{
	    'beforeload':function(loader,node) {
	    	this.baseParams.roleId = com.frontier.gdc.system.noticeRoleManagerV2.roleUserRoleId;
	    }
	},
	//uiProvider 属性
 ...
 * 创建消息配置树
 */
com.frontier.gdc.system.noticeRoleManagerV2.tree = null;
com.frontier.gdc.system.noticeRoleManagerV2.expandNodeArray = [];
com.frontier.gdc.system.noticeRoleManagerV2.createNoticeRoleTree = function(){
	var tree = new Ext.tree.TreePanel({
		layout:'fit',
		...
		loader : com.frontier.gdc.system.noticeRoleManagerV2.loader,
		rootVisible : true,
		autoScroll : true,
		listeners:{
			"checkchange": function(node,checked) {
				var childs = node.childNodes;
				if(childs){
					com.frontier.gdc.system.noticeRoleManagerV2.relatingCheck(node,childs);
				}
			},
			"expandnode": function(node) {
				com.frontier.gdc.system.noticeRoleManagerV2.expandnodeFunc(node);
			},
			"check": function(node, checked) {
				com.frontier.gdc.system.noticeRoleManagerV2.checkFunc(node,checked);
			}
		}
	});
	com.frontier.gdc.system.noticeRoleManagerV2.tree = tree;
	tree.expandAll();
	Ext.Ajax.request({
		url : gdc.webContextRoot + 'noticeRoleTree/queryRoleMessageTime.action',
		params : {"roleId" : com.frontier.gdc.system.noticeRoleManagerV2.roleUserRoleId},
		success : function(result,request) {
			var res = result.responseText;
			var tem = eval('('+res+')');
			if(tem.success==true){
...
//********************************************树事件调用方法***************************************************
//节点关联选择方法
com.frontier.gdc.system.noticeRoleManagerV2.relatingCheck = function(node,childs){
	for(var n=0;n<childs.length;n++){
		childs[n].ui.toggleCheck(node.attributes.checked);
		if(childs[n].expanded){
			var cchilds = childs[n].childNodes;
			if(cchilds){
				com.frontier.gdc.system.noticeRoleManagerV2.relatingCheck(childs[n],cchilds);
			}
		}
	}
}
...
//展开节点调用方法
com.frontier.gdc.system.noticeRoleManagerV2.expandnodeFunc = function(node){
	if(com.frontier.gdc.system.noticeRoleManagerV2.expandNodeArray.indexOf(node)>-1){
		com.frontier.gdc.system.noticeRoleManagerV2.leverChildCheck(node)
		return false;
	}else{
		com.frontier.gdc.system.noticeRoleManagerV2.expandNodeArray.push(node);
		var childs = node.childNodes;
		if(childs){
...
//子节点级联选择调用方法
com.frontier.gdc.system.noticeRoleManagerV2.leverChildCheck = function(node){
	var childs = node.childNodes;
	if(childs){
		for(var n=0;n<childs.length;n++){
			if(childs[n].attributes.checked==true){
				...
				if(childs[n].isExpanded()){
					com.frontier.gdc.system.noticeRoleManagerV2.leverChildCheck(childs[n]);
				}
			}
		}
	}
...
//节点选择调用方法
com.frontier.gdc.system.noticeRoleManagerV2.checkFunc = function(node,checked){
    //获取发送方式
    ...
    for(var i=0;i<sendType.length;i++){
        ...
        if(i==1){
           if(sendType.itemAt(1).checked){
              sendMessageValue = 1;
              com.frontier.gdc.system.noticeRoleManagerV2.isMessage = 1;
           }else{
              sendMessageValue = 0;
              com.frontier.gdc.system.noticeRoleManagerV2.isMessage = 0;
           }
        }
    }
...
//********************************************数据结合树 配合panel**********************************************
//数据结合树 配合panel
com.frontier.gdc.system.noticeRoleManagerV2.leverChildCheck.noticeTypeLevel = new Ext.form.FormPanel({
	height: 90,
	width : 500,
	labelWidth :60,
	items:[
