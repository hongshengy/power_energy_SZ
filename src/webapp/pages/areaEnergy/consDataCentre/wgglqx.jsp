        		...
        		//小储能
	             $('#cxTab').tabs({    
					    ...
					    onSelect:function(title,index){  
					        ...
					        if(index == 0){
					        	var url = basePath+
				          		"/pages/areaEnergy/consDataCentre/subType/wgglqxPic.jsp?LINEID="+lineId
				          		+"&dateTime="+datadate+"&station="+station;
					        	document.getElementById("pictableFrame").src = url;
					        }
					        if(index == 1){
					        	var queryUrl = basePath+"/userDataCenterAction/queryWGGLQXPicData.action?"
					        	+"queryPara.LINE_ID="+lineId+"&queryPara.DATE_TIME="+datadate
					        	+"&queryPara.QUERY_PIC=02&queryPara.QUERY_PARA=02&queryPara.startSize="+station;
	          ...
	          /**$('#cxTab').tabs({height:hheight});**/
	             ...
	             $('#cxTab').tabs({    
					    ...
					    onSelect:function(title,index){  
					        ...
					        if(index == 0){
					        	var url = basePath+
				          		"/pages/areaEnergy/baseData/userDataCenter/wgglqxPic.jsp?LINEID="+lineId
				          		+"&dateTime="+datadate+"&station="+station;
					        	document.getElementById("pictableFrame").src = url;
					        }
					        if(index == 1){
					        	var queryUrl = basePath+"/userDataCenterAction/wgglqxData.action?"
					        	+"queryPara.LINE_ID="+lineId+"&queryPara.DATE_TIME="+datadate
					        	+"&queryPara.QUERY_TYPE=02&queryPara.QUERY_PARA=01&queryPara.startSize="+station;
					        	document.getElementById("sourceFrame").src = queryUrl;
