...
<script type="text/javascript">
    ...
    $.validator.setDefaults({
        submitHandler: function () {
            getDataFnNoErrorF(
                ...
                , function (data) {
                    if (data[0]['MenuInfoMapper.getCountByParams'].length > 1) {
                        window.top.layerMsg('菜单名或url已存在,请重新输入');
                    } else {
                        if(data[0]['MenuInfoMapper.getCountByParams'][0]['id'] != selectRowData['id']){
                            window.top.layerMsg('菜单名或url已存在,请重新输入');
                        }else {
                            subData();
                        }
    ...
    getDataFnNoErrorF(
        dataPath
        , {sql: 'MenuInfoMapper.selectOneInfo', id: selectRowData['id']}
        , function (data) {
            if (null != data[0] && '' != data[0]) {
                appendData(data[0]['MenuInfoMapper.selectOneInfo'][0]);
            }
        }
        , function (data) {
            window.top.layerMsg(data);
    ...
    function subData() {
        ...
        layerConfirm('确认修改？', ['确认', '取消'], [function () {
            getDataFnNoErrorF(
                updateDataPath
                , $('#formSearch').serialize() + '&sql=MenuInfoMapper.updateinfo&id=' + selectRowData['id']
                , function (data) {
                    if (null != data && data[0] == 1) {
                        window.top.layerMsg2('修改成功');
                        parent.query(1, 10);
                    }
                }
                , function (data) {
