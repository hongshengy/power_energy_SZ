...
<script type="text/javascript">
    ...
    var pageDataSql = 'GoodsTypeMapper.getListByParamsByPage';
    ...
    var columns = [
        {
            ...
            field: 'goods_Type_Code',
            title: '物资类型CODE'
        }, {
            field: 'goods_Type_Content',
            title: '物资类型描述'
        }, {
            field: 'creator',
            title: '录入人员'
            ...
            field: 'goods_Type_Status',
            title: '物资类型状态'
        }, {
            field: 'ZHUANGTAI',
            title: '物资类型状态'
        }, {
            field: 'goods_Type_Remark',
            title: '物资类型备注'
        }
    ];
...
//            user_Name: $("#user_Name").val(),
            goods_Type_Name: $("#goods_Type_Name").val(),
    ...
    $('#tb_departments').bootstrapTable('hideColumn', 'goods_Type_Status');     //隐藏需替换的列
...
//    添加
    function add() {
        layerOpenFn('${showPagePath}','添加物资类型','base/goods/typePage',bodyW-100, bodyH);
    }

    //    修改
    function edit() {
        if($('#tb_departments').bootstrapTable('getSelections').length == 1){
            layerOpenFn('${showPagePath}','编辑物资类型', 'base/goods/typeEditPage', bodyW-100, bodyH);
        }else {
    ...
    $('#btn_export').click(function () {
        var mmm = ['物资类型备注','录入人员','物资类型状态','录入日期','','物资类型CODE','物资类型名称','','物资类型描述'];
        var titleParams = mmm.join(",");
        window.open('${exportPath}'+$('#formSearch').serialize()+'&sql=GoodsTypeMapper.selectInfo&fileName=物资类型管理&titleParams='+titleParams);
    });
</script>
