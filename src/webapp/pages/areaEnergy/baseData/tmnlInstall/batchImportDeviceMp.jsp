...
<html>
    ...
    <body scroll='no'>
		...
		<div id='queryDiv' class="container-shadow container-marginTop" style="width:100%;">
            ...
            <form id="thisform" name="thisform" target="fileFrame" method="post"  enctype="multipart/form-data">
            	<input type="hidden" id="deviceName" name="queryPara.deviceName" value="${param.deviceName}"/>
	...
	<script type="text/javascript">
		...
		function downLoadFile(){
			document.thisform.action = "<%=basePath%>/areaEnergy/downloadDeviceExcel.action";
		...
		function checkFile(){
			...
			$('#thisform').form('submit',{
				url :'<%=basePath%>areaEnergy/checkDeviceMpExcelInfo.action',  
				onSubmit: function(){
		...
		function doImport(){
			...
			$.messager.progress();
			$('#thisform').form('submit',{
				url :'<%=basePath%>areaEnergy/doImportDeviceMpExcelInfo.action', 
			    success:function(res){ 
			    	$.messager.progress('close');
			    	if(res == "ok"){
		...
		function queryList(){
			$('#mpImportResultTable').datagrid({
				...
				fitColumns: true,
				url : des.webContextRoot+'areaEnergy/getCheckDeviceMpResult.action',
				sortOrder : 'desc',
				remoteSort : false,
				showFooter : true,
				pageSize : 10,
