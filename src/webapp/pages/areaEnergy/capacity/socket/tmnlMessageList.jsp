...
<html>
	...
	function doEdit(){
		...
		opener.$('#terminalAssetNo').textbox('setValue',row.TMNL_ASSET_NO) ;
		opener.$('#terminalId').attr('value',row.TERMINAL_ID) ;
		window.close();
	}
	function cancel(){
		window.close();
	...
	$(function() {
		...
		$('#collArchiveTable').datagrid({
			...
			url : des.webContextRoot +'capacity/queryCapacityGateway.action',
			queryParams : {/* "queryPara.subId" : subId,  */"queryPara.consId" : consId},
			sortOrder : 'desc',
			remoteSort : false,
			showFooter : true,
			pageSize : 50,
