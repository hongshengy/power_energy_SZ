...
<script type="text/javascript">
    ...
    var pageDataSql = 'DogWorkLogInfoMapper.getListByParamsByPage';
...
//        debugger;
    ...
    //    添加
    function add() {
        layerOpenFn('${showPagePath}','添加检疫犬工作日志','base/beon/dog_work_log_infoPage',bodyW-100, bodyH);
    }

//    编辑
    function editFun(id) {
        parentParams = id;
        layerOpenFn('${showPagePath}','编辑检疫犬工作日志','base/beon/dog_work_log_infoEditPage',bodyW-100, bodyH);
    }
    ...
    function delFun(id) {
        ...
        layerConfirm('确认删除？',['确认','取消'],[function () {
            getDataFnNoErrorF(
                deleteDataPath
                , {sql:'DogWorkLogInfoMapper.deleteInfo',id:id}
                , function (data) {
    ...
    $('#btn_export').click(function () {
        var mmm = ['检疫犬名','','说明','','物品名称','备注','ID','检疫犬图片路径','领取时间','物品类型','检疫犬ID','领取人'];
        var titleParams = mmm.join(",");
        window.open('${exportPath}'+$('#formSearch').serialize()+'&sql=DogWorkLogInfoMapper.selectInfo&fileName=检疫犬工作日志管理&titleParams='+titleParams);
    });
</script>
<%--导航--%>
