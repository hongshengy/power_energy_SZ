        ...
        <div class="panel-body">
            <form id="formSearch" class="form-horizontal">
                <div class="form-group" style="margin-top:15px">
                    <label class="control-label col-sm-1" for="storage_Room_Name">库房名</label>
                    <div class="col-sm-3">
...
<script type="text/javascript">
    ...
    var pageDataSql = 'GoodsStorageroomInfoMapper.getListByParamsByPage';
    ...
    var columns = [
        {
            ...
            field: 'storage_Room_Code',
            title: '库房CODE'
        }, {
            field: 'storage_Room_Content',
            title: '库房描述'
        }, {
            field: 'creator',
            title: '录入人员'
            ...
            field: 'storage_Room_Type_Status',
            title: '库房状态'
        }, {
            field: 'ZHUANGTAI',
            title: '库房状态'
...
//            user_Name: $("#user_Name").val(),
            storage_Room_Name: $("#storage_Room_Name").val(),
    ...
    $('#tb_departments').bootstrapTable('hideColumn', 'storage_Room_Type_Status');     //隐藏需替换的列
    $('#tb_departments').bootstrapTable('hideColumn', 'creator');     //隐藏需替换的列
...
//    父子表gird显示参照fatherAndSonGird.jsp
    ...
    //    添加
    function add() {
        layerOpenFn('${showPagePath}','添加库房','base/goods/storage_roomPage',bodyW-100, bodyH);
    }

    //    修改
    function edit() {
        if($('#tb_departments').bootstrapTable('getSelections').length == 1){
            layerOpenFn('${showPagePath}','编辑库房', 'base/goods/storage_roomEditPage', bodyW-100, bodyH);
        }else {
    ...
    //    删除
    function deleteFun() {
        sysDeleteFun('GoodsTypeMapper.deleteInfo');
    }

    //    导出
    $('#btn_export').click(function () {
        var mmm = ['库房名称','','','库房状态','库房CODE','录入日期','录入人员','','库房描述'];
        var titleParams = mmm.join(",");
        window.open('${exportPath}'+$('#formSearch').serialize()+'&sql=GoodsStorageroomInfoMapper.selectInfo&fileName=库房管理&titleParams='+titleParams);
    });
</script>
