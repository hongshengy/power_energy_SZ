...
<div class="panel-body" style="padding:5px;">
    <div class="panel panel-default">
        <div class="panel-heading">检疫犬退役管理</div>
...
<script type="text/javascript">
    ...
    var pageDataSql = 'RetireInfoMapper.getListByParamsByPage';
    ...
    var columns = [
        {
            ...
            field: 'approver_User_Name',
            title: '批准人'
        }, {
            field: 'apply_organization',
            title: '申请单位'
        }
    ];
...
//    添加
    function add() {
        layerOpenFn('${showPagePath}','添加检疫犬退役','base/retire/retire_infoPage',bodyW-100, bodyH);
    }

    //    修改
    function edit() {
        if($('#tb_departments').bootstrapTable('getSelections').length == 1){
            layerOpenFn('${showPagePath}','编辑检疫犬退役', 'base/retire/retire_infoEditPage', bodyW-100, bodyH);
        }else {
    ...
    //    删除
    function deleteFun() {
        sysDeleteFun('RetireInfoMapper.deleteInfo');
    }

    //    导出
    $('#btn_export').click(function () {
        var mmm = ['退役原因','申请单位','ID','退役时间','批准人','批准部门','检疫犬ID'];
        var titleParams = mmm.join(",");
        window.open('${exportPath}'+$('#formSearch').serialize()+'&sql=RetireInfoMapper.selectInfo&fileName=检疫犬退役管理&titleParams='+titleParams);
    });
</script>
