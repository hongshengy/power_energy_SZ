        ...
        <div class="panel-body">
            <form id="formSearch" class="form-horizontal">
                <div class="form-group" style="margin-top:15px">
                    <label class="control-label col-sm-1" for="kennel_Name">犬舍名称</label>
                    <div class="col-sm-3">
...
<script type="text/javascript">
    ...
    var pageDataSql = 'KennelMapper.getListByParamsByPage';
...
//            user_Name: $("#user_Name").val(),
    ...
    $('#tb_departments').bootstrapTable('hideColumn', 'kennel_Status');     //隐藏需替换的列
...
//    添加
    function add() {
        layerOpenFn('${showPagePath}','添加犬舍','base/kennel/kennelPage',bodyW-100, bodyH);
    }

    //    修改
    function edit() {
        if($('#tb_departments').bootstrapTable('getSelections').length == 1){
            layerOpenFn('${showPagePath}','编辑犬舍', 'base/kennel/kennelEditPage', bodyW-100, bodyH);
        }else {
    ...
    //    删除
    function deleteFun() {
        sysDeleteFun('KennelMapper.deleteInfo');
    }

    //    导出
    $('#btn_export').click(function () {
        var mmm = ['犬舍名称','状态','备注','创建时间','','犬舍说明','','犬舍编号'];
        var titleParams = mmm.join(",");
        window.open('${exportPath}'+$('#formSearch').serialize()+'&sql=KennelMapper.selectInfo&fileName=犬舍管理&titleParams='+titleParams);
    });
</script>
