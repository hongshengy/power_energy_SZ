...
<script type="text/javascript">
    ...
    var selectRowData = parent.selectRowData;
    ...
    $.validator.setDefaults({
        submitHandler: function () {
            getDataFnNoErrorF(
                ...
                , function (data) {
                    if (data[0]['OrganizationInfoMapper.getCountByParams'].length > 1) {
                        window.top.layerMsg('组织机构名或机构代码已存在,请重新输入');
                    } else {
                        if(data[0]['OrganizationInfoMapper.getCountByParams'][0]['id'] != selectRowData['id']){
                            window.top.layerMsg('组织机构名或机构代码已存在,请重新输入');
                        }else {
                            subData();
                        }
    ...
    function subData() {
        layerConfirm('确认提交？', ['确认', '取消'], [function () {
            getDataFnNoErrorF(
                updateDataPath
                , $('#formSearch').serialize() + '&sql=OrganizationInfoMapper.updateinfo&id='+selectRowData['id']
                , function (data) {
