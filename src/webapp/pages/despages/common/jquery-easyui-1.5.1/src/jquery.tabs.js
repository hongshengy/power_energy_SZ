...
//					selectTab(container, getLiIndex(li));
	...
	function createTab(container, options, pp) {
		...
		pp.panel($.extend({}, options, {
			...
			onBeforeOpen: function(){
				...
				// scroll the tabs to center position if required.
	...
	function addTab(container, options) {
		...
		if (options.selected){
			selectTab(container, options.index);	// select the added tabs panel
		}
	}
	
	/**
	 * update tabs panel, param has following properties:
	 * tabs: the tabs panel to be updated
	 * options: the tabs panel options
	...
	function updateTab(container, param){
		...
		var pp = param.tab;	// the tabs panel
		var opts = pp.panel('options');	// get the tabs panel options
	...
	/**
	 * close a tabs with specified index or title
	 */
...
//		setScrollers(container);
	...
	/**
	 * do first select action, if no tabs is setted the first tabs will be selected.
	 */
	...
	$.fn.tabs.methods = {
		...
		scrollBy: function(jq, deltaX){	// scroll the tabs header by the specified amount of pixels
			return jq.each(function(){
	...
	$.fn.tabs.defaults = {
		width: 'auto',
		height: 'auto',
		headerWidth: 150,	// the tabs header width, it is valid only when tabPosition set to 'left' or 'right' 
		tabWidth: 'auto',	// the tabs width
		tabHeight: 27,		// the tabs height
		selected: 0,		// the initialized selected tabs index
		showHeader: true,
		plain: false,
		fit: false,
		border: true,
