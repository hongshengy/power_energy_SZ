...
<script type="text/javascript">
    ...
    var pageDataSql = 'RetireAdoptMapper.getListByParamsByPage';
...
//    添加
    function add() {
        layerOpenFn('${showPagePath}','添加退役检疫犬领养','base/retire/retire_adoptPage',bodyW-100, bodyH);
    }

    //    修改
    function edit() {
        if($('#tb_departments').bootstrapTable('getSelections').length == 1){
            layerOpenFn('${showPagePath}','编辑退役检疫犬领养', 'base/retire/retire_adoptEditPage', bodyW-100, bodyH);
        }else {
    ...
    //    删除
    function deleteFun() {
        sysDeleteFun('RetireAdoptMapper.deleteInfo');
    }

    //    导出
    $('#btn_export').click(function () {
        var mmm = ['批准人','领养人','领养时间','检疫犬ID','领养人地址','领养人电话','申请单位','ID'];
        var titleParams = mmm.join(",");
        window.open('${exportPath}'+$('#formSearch').serialize()+'&sql=RetireAdoptMapper.selectInfo&fileName=退役检疫犬领养管理&titleParams='+titleParams);
    });
</script>
